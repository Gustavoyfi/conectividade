...
class Send(threading.Thread):
    def __init__(self, sock, name, running_flag, recipient_flag):
        super().__init__()
        self.sock = sock
        self.name = name
        self.running_flag = running_flag
        self.recipient_flag = recipient_flag  # Define o destinatário (modo privado)
    ...
    def run(self):
        while self.running_flag.is_set():
            ...
            if message == "QUIT":
                ...
                self.recipient_flag[0] = recipient
                print(f"Conversa privada com {recipient} iniciada.")
            elif message == "/endprivate":
                self.recipient_flag[0] = None
                print("Conversa privada encerrada.")
            else:
                if self.recipient_flag[0]:  # Envia como mensagem privada
                    self.sock.sendall(f"/private {self.recipient_flag[0]} {message}".encode("ascii"))
                else:  # Envia como mensagem pública
...
class Client:
    def __init__(self, host, port):
        ...
        self.recipient_flag = [None]  # Guarda o destinatário privado
    ...
    def start(self):
        ...
        # Solicita um nome de usuário até ser aceito
        while True:
        ...
        send = Send(self.sock, self.name, self.running_flag, self.recipient_flag)
